...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   raycaster_player_view.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tdeliot <tdeliot@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/28 10:51:53 by tdeliot           #+#    #+#             */
/*   Updated: 2025/07/28 10:52:29 by tdeliot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_raycast	*raycasting_player_view(const t_player *player, const t_map *map)
{
	...
	while (i < NBR_RAY)
	{
		modify_raycasting_angle(&caster, player, i);
		rays[i] = raycasting(player, map, caster.angle_current);
		correct_fisheye(&caster, player, i, rays);
		i++;
	}
	return (rays);
}
...
void	correct_fisheye(t_multiple_raycast *caster,
			const t_player *player, int i, t_raycast	*rays)
{
	caster->angle_diff = 
