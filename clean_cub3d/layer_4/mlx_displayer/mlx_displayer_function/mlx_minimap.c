...
/*   Updated: 2025/07/11 11:23:59 by tdeliot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_a_case(int tile_size, t_mlx_displayer *displayer, t_pixel_data *px)
{
	int	dy;
	int	dx;
...
// tile_size is the size of a case. we want that the map always goes into the minimap,
// the size of the case depend of the defining size of the minimap.
void	minimap_background(t_mlx_displayer *displayer, const t_map *map, int tile_size)
{
	t_pixel_data px;
...
void minimap_player(t_mlx_displayer *displayer, const t_map *map, int tile_size, const t_player *player)
{
	int	px;
	int	py;
	int dy;
...
void mlx_display_minimap(t_i_displayer *self, const t_map *map, const t_player *player)
{
	...
	tile_size = init_tile_size(map, disp->minimap_img.height, disp->minimap_img.width);
