...
/*   Updated: 2025/07/14 10:27:57 by tdeliot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// this function look at the size of the screen to calcule the lenght line
...
void	init_line_to_draw_texture(t_line_to_display *line_to_display,
			const t_framebuffer *frame_buff, const t_mlx_displayer *displayer,
			int i)
{
...
// this function help us to decide which par of the texture 
// will be drawn on the line. for that we look were the point 
// touch the wall. (4.5), we keep the .5, then we look the corresponding 
// point in the texture, (in this case texture->width* 0.5)
									...
									const t_framebuffer *frame_buff, int i)
{
	...
	wall_x -= floor(wall_x);
	line_to_display->texture_x = 
		(int)(wall_x * line_to_display->texture->width);
...
void	init_line_to_draw_y_texture(t_line_to_display *line_to_display, int y)
{
	...
	line_to_display->texture_y = ((d * line_to_display->texture->height) 
			/ line_to_display->line_height) / 256;
...
void	init_line_to_draw_pixel_color(t_line_to_display *line_to_display)
{
	line_to_display->pixel = line_to_display->texture->img_pixels_ptr + 
		...
		line_to_display->texture_x * 
		(line_to_display->texture->bits_per_pixel / 8); 
