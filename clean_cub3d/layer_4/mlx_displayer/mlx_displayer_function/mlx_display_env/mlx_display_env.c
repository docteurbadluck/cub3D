...
/*   Updated: 2025/07/14 10:23:50 by tdeliot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_the_line(const t_mlx_displayer *displayer,
			t_line_to_display *line_to_display, int i);
			...
			const t_framebuffer *frame_buff)
{
	...
	background_render(displayer->sky_color,
		displayer->ground_color, displayer->mlx);
	while (i < NBR_RAY)
	{
		init_line_to_draw_boundary(&line_to_display, frame_buff, i);
		init_line_to_draw_texture(&line_to_display, frame_buff,
			displayer, i);
...
//for each pixel of the line, we are searching
// the correspondent into the texture that we defined.
void	draw_the_line(const t_mlx_displayer *displayer,
			t_line_to_display *line_to_display, int i)
{
	int		screen_x;
